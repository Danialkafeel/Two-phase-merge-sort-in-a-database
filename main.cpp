...
public:
	...
	Two_phase_MS(string f1, string f2, string main_mem_size){
		inputFile = f1;
		outputFile = f2;
		// try{
		...
		// }
		// catch(...){
		// 	cout<<"Main Memory must be an integer or float\nAborting...";
		// 	throw -1;
		// }
		this->main_mem_size = stod(main_mem_size)*1000000;
		finn.open(inputFile);
		...
		chunk_size = (this->main_mem_size)/tuple_size;
	...
	public:
		...
		bool operator()(vector<string> v1,vector<string> v2){
			for(int col:this->sort_cols){
				if(v1[col] != v2[col]){
					// cout<<"v1,v2 col = "<<v1[col]<<","<<v2[col]<<endl;
					if(this->asc)
	...
	void Sort(bool ascd, vector<string> sort_cols){
		...
		ll temp = main_mem_size/tuple_size;
...
int main(int argc, char* argv[]) 
{
	...
	string inputFile, outputFile, main_mem_size, asc;
	inputFile = argv[1];
	outputFile = argv[2];
	...
	Two_phase_MS obj(inputFile, outputFile, main_mem_size);
